...
{
    public class MED07_0000Repository
    {
        Comm comm = new Comm();
        ...
        /// < returns > DTO MED07_0000</returns>
        public MED07_0000 GetDTO(string pTkCode)
        {
            MED07_0000 datas = new MED07_0000();
            string sSql = "";

            if (string.IsNullOrEmpty(pTkCode))
            {
                sSql = "SELECT * FROM MED07_0000";
            }
            else
            {
                sSql = "SELECT * FROM MED07_0000 where med07_0000=@med07_0000";
            }
            ...
            using (SqlConnection con_db = comm.Set_DBConnection())
            {
                ...
                sqlCommand.Parameters.Add(new SqlParameter("@med07_0000", pTkCode));
                ...
                if (reader.HasRows)
                {
                    while (reader.Read())
                    {
                        datas = new MED07_0000
                        {

                            med07_0000 = comm.sGetInt32(reader["med07_0000"].ToString()),
        ...
        /// </summary>
        /// <param name = "pTkCode" > 鍵值 / 傳空值取全部資料 </ param >
        /// < returns > List MED07_0000</returns>
        public List<MED07_0000> Get_DataList(string pTkCode)
        {
            ...
            using (SqlConnection con_db = comm.Set_DBConnection())
            {
                ...
                while (reader.Read())
                {
                    ...
                    data.med07_0000 = comm.sGetInt32(reader["med07_0000"].ToString());
        ...
        /// <returns></returns>
        public List<MED07_0000> Get_DataList(string pUsrCode, string pPrgCode)
        {
            string sLimitStr = comm.Get_LimitByUsrCode(pUsrCode, pPrgCode);
            string sLockGrpCode = comm.Get_QueryData("BDP00_0000", "lock_med07_0000", "par_name", "par_value");
            ...
            using (SqlConnection con_db = comm.Set_DBConnection())
            {
                ...
                while (reader.Read())
                {
                    ...
                    //資料邏輯刪除、修改
                    //if (arr_LockGrpCode.Contains(data.med07_0000)) {
                    //    data.can_delete = "N";
                    //    data.can_update = "N";
                    //}
        ...
        /// <returns></returns>
        public List<MED07_0000> Get_DataListByQuery(string pUsrCode, string pPrgCode, string pWhere)
        {
            List<MED07_0000> list = new List<MED07_0000>();

            string sSql = " SELECT MED07_0000.*, MEB15_0000.mac_name, MEB20_0000.pro_name,   " +
                          " MEB29_0000.station_name as station_name,"+
                          " MEB30_0000.work_name as work_name"+
                          " FROM MED07_0000 " +
                          " left join MEB29_0000 on MEB29_0000.station_code = MED07_0000.station_code"+
                          " left join MEB30_0000 on MEB30_0000.work_code = MED07_0000.work_code"+
                          " left join MEB15_0000 on MEB15_0000.mac_code = MED07_0000.mac_code " +
                          " left join MEB20_0000 on MEB20_0000.pro_code  = MED07_0000.pro_code " ;
                          //" left join BDP08_0000 on BDP08_0000.usr_code   = MED07_0000.end_usr_code "

            // 取得資料
            list = comm.Get_ListByQuery<MED07_0000>(sSql, pWhere, pUsrCode, pPrgCode);
        ...
        /// <summary>
        /// 傳入一個MED07_0000的DTO，存檔，一次存檔一筆
        /// </summary>
        /// <param name="MED07_0000">DTO</param>
        public void InsertData(MED07_0000 MED07_0000)
        {
            string sSql = "INSERT INTO " +
                          " MED07_0000 (  mo_code,  wrk_code, work_code, station_code, mac_code,  pro_code,  lot_no,  pro_qty,          " +
                          "                         pro_unit,  loc_code,  ins_date,  ins_time,  usr_code,  des_memo,       " +
            ...
            using (SqlConnection con_db = comm.Set_DBConnection())
            {
                ...
                //SqlCommand sqlCommand = new SqlCommand(sSql);
                //sqlCommand.Connection = con_db;
                //sqlCommand.Parameters.Add(new SqlParameter("@med07_0000", MED07_0000.med07_0000));
                //sqlCommand.Parameters.Add(new SqlParameter("@med07_0000", MED07_0000.med07_0000));
                //sqlCommand.Parameters.Add(new SqlParameter("@sup_type_name", MED07_0000.sup_type_name));
                //sqlCommand.ExecuteNonQuery();
            }
        }

        /// <summary>
        /// 傳入一個MED07_0000的DTO，修改，一次修改一筆
        /// </summary>
        /// <param name="MED07_0000">DTO</param>
        ...
        {
            string sSql = "DELETE FROM MED07_0000 WHERE med07_0000 = @med07_0000;";
            //sSql += " Delete from BDP09_0100 where med07_0000 = @med07_0000; ";
            using (SqlConnection con_db = comm.Set_DBConnection())
            {
                con_db.Execute(sSql, new { med07_0000 = pTkCode });
        ...
        {
            string sSql = "Update MEP02_0000 set rtn_qty= rtn_qty +@rtn_qty,total_qty=total_qty -@total_qty" +
                          " where pro_code =@pro_code"+
                          "   and wrk_code =@wrk_code;";
            //sSql += " Delete from BDP09_0100 where med06_0000 = @med06_0000; ";
            using (SqlConnection con_db = comm.Set_DBConnection())
            {
                con_db.Execute(sSql, new { rtn_qty = pProQty, pro_code = pProCode, wrk_code = pWrkCode , total_qty = pProQty });

            }

        }
        /// <summary>
        /// 傳入MED07索引值、結案時間，更新MED06結案時間
        /// </summary>
        /// <param name="MED07_0000"></param>
        /// <param name="pEndDate"></param>
        /// <param name="pEndTime"></param>
        public void UpdataMED07EndTime(string MED07_0000, string pEndDate, string pEndTime)
        {
            string sSql = "Update MED07_0000 set end_date= @end_date , end_time =@end_time " +
                          " where med07_0000=@med07_0000;";
            using (SqlConnection con_db = comm.Set_DBConnection())
            {
                con_db.Execute(sSql, new { end_date = pEndDate, end_time = pEndTime, med07_0000 = MED07_0000 });
            }
         ...
         * 暫存DataTable參考
        /// <summary>
        /// 取得MED07_0000角色的DataTable
        /// </summary>
        /// <param name="pTkCode">有傳鍵值取一筆，鍵值空白取全部</param>
        /// <returns></returns>
        public DataTable GetMED07_0000_dt(string pTkCode)
        {
            DataTable dtTmp = new DataTable();

            DataTable dtDat = new DataTable();
            dtDat.Columns.Add("med07_0000", System.Type.GetType("System.String"].ToString());
            dtDat.Columns.Add("med07_0000", System.Type.GetType("System.String"].ToString());
            ...
            {
                sSql = "SELECT * FROM MED07_0000 where med07_0000='" + pTkCode + "'";
            }
            ...
            for (i = 1; i < dtTmp.Rows.Count - 1; i++)
            {
                ...
                drow["med07_0000"] = dtTmp.Rows[i]["med07_0000"];
                drow["med07_0000"] = dtTmp.Rows[i]["med07_0000"];
