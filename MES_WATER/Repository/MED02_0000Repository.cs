...
{
    public class MED02_0000Repository
    {
        Comm comm = new Comm();
        ...
        /// < returns > DTO MED02_0000</returns>
        public MED02_0000 GetDTO(string pTkCode)
        {
            MED02_0000 datas = new MED02_0000();
            string sSql = "";

            if (string.IsNullOrEmpty(pTkCode))
            {
                sSql = "SELECT * FROM MED02_0000";
            }
            else
            {
                sSql = "SELECT * FROM MED02_0000 where med02_0000=@med02_0000";
            }
            ...
            using (SqlConnection con_db = comm.Set_DBConnection())
            {
                ...
                sqlCommand.Parameters.Add(new SqlParameter("@med02_0000", pTkCode));
                ...
                if (reader.HasRows)
                {
                    while (reader.Read())
                    {
                        datas = new MED02_0000
                        {

                            med02_0000 = comm.sGetInt32(reader["med02_0000"].ToString()),
                            ...
                            mo_status_wrk = comm.sGetString(reader["mo_status_wrk"].ToString()),
        ...
        /// </summary>
        /// <param name = "pTkCode" > 鍵值 / 傳空值取全部資料 </ param >
        /// < returns > List MED02_0000</returns>
        public List<MED02_0000> Get_DataList(string pTkCode)
        {
            List<MED02_0000> list = new List<MED02_0000>();
            string sSql = "";
            ...
            using (SqlConnection con_db = comm.Set_DBConnection())
            {
                ...
                while (reader.Read())
                {
                    MED02_0000 data = new MED02_0000();

                    data.med02_0000 = comm.sGetInt32(reader["med02_0000"].ToString());
                    ...
                    data.mo_status_wrk = comm.sGetString(reader["mo_status_wrk"].ToString());
        ...
        /// <returns></returns>
        public List<MED02_0000> Get_DataList(string pUsrCode, string pPrgCode)
        {
            string sLimitStr = comm.Get_LimitByUsrCode(pUsrCode, pPrgCode);
            string sLockGrpCode = comm.Get_QueryData("BDP00_0000", "lock_med02_0000", "par_name", "par_value");
            ...
            using (SqlConnection con_db = comm.Set_DBConnection())
            {
                ...
                while (reader.Read())
                {
                    ...
                    //資料邏輯刪除、修改
                    //if (arr_LockGrpCode.Contains(data.med02_0000)) {
                    //    data.can_delete = "N";
                    //    data.can_update = "N";
                    //}
        ...
        /// <returns></returns>
        public List<MED02_0000> Get_DataListByQuery(string pUsrCode, string pPrgCode, string pWhere)
        {
            List<MED02_0000> list = new List<MED02_0000>();

            string sSql = " SELECT MED02_0000.*, MEB15_0000.mac_name, BDP21_0100.field_name as mo_status_wrk_name, BDP08_0000.usr_name " +
                          " FROM MED02_0000 " +
                          " left join MEB15_0000 on MEB15_0000.mac_code = MED02_0000.mac_code " +
                          " left join BDP21_0100 on BDP21_0100.field_code = MED02_0000.mo_status_wrk and BDP21_0100.code_code = 'mo_status_wrk' " +
                          " left join BDP08_0000 on BDP08_0000.usr_code = MED02_0000.usr_code " ;

            // 取得資料
            list = comm.Get_ListByQuery<MED02_0000>(sSql, pWhere, pUsrCode, pPrgCode);
        ...
        /// <summary>
        /// 傳入一個MED02_0000的DTO，存檔，一次存檔一筆
        /// </summary>
        /// <param name="MED02_0000">DTO</param>
        public void InsertData(MED02_0000 MED02_0000)
        {
            string sSql = "INSERT INTO " +
                          " MED02_0000 (  mo_code,   mac_code,  mo_status_wrk,  ins_date,  ins_time,  usr_code ) " +
                          "     VALUES ( @mo_code,  @mac_code, @mo_status_wrk, @ins_date, @ins_time, @usr_code ) " ;
            using (SqlConnection con_db = comm.Set_DBConnection())
            {
                con_db.Execute(sSql, MED02_0000);
                //SqlCommand sqlCommand = new SqlCommand(sSql);
                //sqlCommand.Connection = con_db;
                //sqlCommand.Parameters.Add(new SqlParameter("@med02_0000", MED02_0000.med02_0000));
                //sqlCommand.Parameters.Add(new SqlParameter("@med02_0000", MED02_0000.med02_0000));
                //sqlCommand.Parameters.Add(new SqlParameter("@sup_type_name", MED02_0000.sup_type_name));
                //sqlCommand.ExecuteNonQuery();
            }
        }

        /// <summary>
        /// 傳入一個MED02_0000的DTO，修改，一次修改一筆
        /// </summary>
        /// <param name="MED02_0000">DTO</param>
        ...
        {
            string sSql = "DELETE FROM MED02_0000 WHERE med02_0000 = @med02_0000;";
            //sSql += " Delete from BDP09_0100 where med02_0000 = @med02_0000; ";
            using (SqlConnection con_db = comm.Set_DBConnection())
            {
                con_db.Execute(sSql, new { med02_0000 = pTkCode });
        ...
        /// 更新MED02_0000強制結案資料
        /// </summary>
        /// <param name="MED02_0000"></param>
        public void InsertEndData(MED02_0000 MED02_0000)
        {
            string sSql = "INSERT INTO " +
                          " MED02_0000 (  mo_code,   mac_code,  mo_status_wrk,  ins_date,  ins_time,  usr_code, des_memo, is_ng, is_end, end_memo, end_date, end_time, end_usr_code ) " +
                          "     VALUES ( @mo_code,  @mac_code, @mo_status_wrk, @ins_date, @ins_time, @usr_code , @des_memo, @is_ng, @is_end, @end_memo, @end_date, @end_time, @end_usr_code) ";
            using (SqlConnection con_db = comm.Set_DBConnection())
            {
                con_db.Execute(sSql, MED02_0000);
                //SqlCommand sqlCommand = new SqlCommand(sSql);
         ...
         * 暫存DataTable參考
        /// <summary>
        /// 取得MED02_0000角色的DataTable
        /// </summary>
        /// <param name="pTkCode">有傳鍵值取一筆，鍵值空白取全部</param>
        /// <returns></returns>
        public DataTable GetMED02_0000_dt(string pTkCode)
        {
            DataTable dtTmp = new DataTable();

            DataTable dtDat = new DataTable();
            dtDat.Columns.Add("med02_0000", System.Type.GetType("System.String"].ToString());
            dtDat.Columns.Add("med02_0000", System.Type.GetType("System.String"].ToString());
            ...
            {
                sSql = "SELECT * FROM MED02_0000 where med02_0000='" + pTkCode + "'";
            }
            ...
            for (i = 1; i < dtTmp.Rows.Count - 1; i++)
            {
                ...
                drow["med02_0000"] = dtTmp.Rows[i]["med02_0000"];
                drow["med02_0000"] = dtTmp.Rows[i]["med02_0000"];
