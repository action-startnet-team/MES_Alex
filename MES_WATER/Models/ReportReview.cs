...
namespace MES_WATER.Models
{
    public class ReportReview
    {
        ...
        /// </summary>
        /// <returns></returns>
        public string Get_ReportGroupCode(string pReportCode)
        {
            string sNewTkCode = "";
            string sSql = "select max(right(report_group_code,6)) as tk_code" +
                          "  from RSS03_0000" +
                          " where report_group_code like '" + pReportCode + "%'";
            var dtTmp = comm.Get_DataTable(sSql);
            if (dtTmp.Rows.Count > 0)
            {
                string sTkCode = dtTmp.Rows[0]["tk_code"].ToString();
                int iNewScrNo = comm.sGetInt32(sTkCode) + 1;
                sNewTkCode = pReportCode + "-" + iNewScrNo.ToString().PadLeft(6, '0');
            }
            else
            {
                sNewTkCode = pReportCode + "-000001";
            }
            return sNewTkCode;
        }
        ...
        /// 新增下一位待審人員
        /// </summary>
        /// <param name="Key">報表集成群組碼</param>
        public void Ins_ReviewByReportGroup(string pReportGroupCode, string pUsrCode)
        {
            object data = new object();
            string sReportCode = comm.Get_Data("RSS03_0000", pReportGroupCode, "report_group_code", "report_code");
            string sReportType = comm.Get_Data("RSS03_0000", pReportGroupCode, "report_group_code", "report_type");
            ...
            switch (sReportType) {
                case "A":
                    sReviewCode = comm.Get_QueryData("EPB04_0000", sEpbCode, "epb_code", "review_code");
                    break;
                case "B":
                    sReviewCode = comm.Get_QueryData("EPB04_0000", sReportCode, "report_code", "review_code");
                    break;
            }          

            //檢查此表單是否有審核設定作業
            if (!string.IsNullOrEmpty(sReviewCode))
            {
                data = new
                {
                    review_code = sReviewCode,
                    epb_code = sEpbCode,
                    report_group_code = pReportGroupCode,
                    is_ok = "P",
                    scr_no = Get_ReviewScrNo(pReportGroupCode),
        ...
        {
            string sSql = "select report_group_code from EPB05_0000 " +
                          " where result_code = '99' " +
                          "   and epb_code = @epb_code";
            var dtTmp = comm.Get_DataTable(sSql, "epb_code", pEpbCode);
            return GD.DataFieldToStr(dtTmp, "report_group_code");
        }


        public bool Chk_ReportGroupIsPass(string pReportGroupCode)
        {
            bool val = false;
            string sSql = "select report_group_code from EPB05_0000 " +
                          " where result_code = '99' " +
                          "   and report_group_code = @report_group_code";
            var dtTmp = comm.Get_DataTable(sSql, "report_group_code", pReportGroupCode);
            if (dtTmp.Rows.Count > 0) {
                val = true;
            }
            return val;
        ...
        /// 取得目前審核資料後面未審的人員
        /// </summary>
        /// <param name="pReportGroupCode"></param>
        /// <returns></returns>
        public string Get_HigherReviewUsr(string pReportGroupCode)
        {
            string sSql = "";
            DataTable dtTmp = new DataTable();
            //審核代號
            string sReviewCode = Get_FinalReviewData(pReportGroupCode, "review_code");

            //目前進度的審核人員(如果該資料已結案，則此人員為決行人員)
            string sUsrCode = Get_FinalReviewData(pReportGroupCode, "usr_code");

            //審核人員的審核層級
            string sReviewLevel = Get_UsrReviewLevel(sReviewCode, sUsrCode);

            sSql = "select * " +
                   "  from EPB04_0100" +
                   " where review_code = '"+ sReviewCode + "'" +
                   "   and review_level > '"+ sReviewLevel + "'" +
                   "   and usr_code <> ''" +
                   " order by review_level";
            dtTmp = comm.Get_DataTable(sSql);
            return GD.DataFieldToStr(dtTmp,"usr_code");
        }
        ...
        public bool Chk_UsrIsReviewerOfEpb(string pEpbCode,string pUsrCode) {
            bool val = false;
            string sSql = "select  * " +
                          "  from EPB04_0000" +
        ...
        public bool Chk_UsrIsReviewerOfReport(string pReportCode, string pUsrCode)
        {
            bool val = false;
            string sSql = "select  * " +
                          "  from EPB04_0000" +
                          "  left join  EPB04_0100 on EPB04_0000.review_code = EPB04_0100.review_code" +
                          " where report_code = '" + pReportCode + "'" +
                          "   and usr_code = '" + pUsrCode + "'";
            var dtTmp = comm.Get_DataTable(sSql);
            if (dtTmp.Rows.Count > 0)
            {
